...
from torchsig.utils import SignalData
        ...
        if batch_size == 1:
            ...
            for i in tqdm(range(self.length)):
                ...
                with self._env.begin(write=True) as txn:
                    ...
                    txn.put(str(i).encode(), str(annotation).encode(), db=self._annotation_db)
            ...
            for batch_idx in tqdm(range(num_batches)):
                ...
                for batch_sample_idx in range(batch_size):
                    process_index.append((int(batch_idx*batch_size+batch_sample_idx), cfg))
                ...
                for data, annotation in result:
                    data_c64 = data.astype(np.complex64)
                    with self._env.begin(write=True) as txn:
                        txn.put(str(lmdb_idx).encode(), pickle.dumps(data_c64), db=self._sample_db)
                        txn.put(str(lmdb_idx).encode(), str(annotation).encode(), db=self._annotation_db)
                    lmdb_idx += 1
