...
def seed_worker() -> None:
    """Initializes the random seeds for a DataLoader worker.

    This function is intended to be used as the `worker_init_fn` in a
    `torch.utils.data.DataLoader`. It ensures that each worker process,
    along with any `Seedable` objects (like datasets or transforms) used
    within that worker, gets a unique random seed derived from the main
    process using `torch.utils.data.get_worker_info()`.
    """
    worker_info = torch.utils.data.get_worker_info()
    if worker_info is not None:
        # Get dataset instance for this worker
        dataset = worker_info.dataset
        # Use the seed generated by DataLoader for this worker
        worker_seed = worker_info.seed % 2**32
        # Seed the dataset instance if it's Seedable
        # This should propagate the seed to child Seedable objects (e.g., transforms)
        if isinstance(dataset, Seedable):
            dataset.seed(worker_seed)
        # Also seed global numpy and random for safety/compatibility, although
        # seeding the Seedable dataset should be the primary mechanism for torchsig.
        np.random.seed(worker_seed)
        random.seed(worker_seed)
    # else: main process or num_workers=0, seeding is handled elsewhere
    ...
    def __init__(
        ...
        **kwargs  # any additional file handler args
        ...
        if dataset.dataset_metadata.num_samples is None:
            raise ValueError(
                "Must specify num_samples as an integer number. Cannot write infinite dataset to disk.")
        ...
        self.dataloader = DataLoader(
            ...
            worker_init_fn=seed_worker  # Pass the worker init function
        )
    ...
    def create(self) -> None:
        ...
        if self.writer.exists() and not self.overwrite:
            ...
            if len(different_params) == 0:
                print(
                    f"Dataset already exists in {self.writer.root}. Not regenerating.")
                return
            # else:
            ...
            print(
                f"Dataset exists at {self.writer.root} but is different than current dataset.")
            print("Differences:")
        ...
        # generate info yamls
        write_dict_to_yaml(f"{self.writer.root}/{dataset_yaml_name}",
                           self.dataloader.dataset.dataset_metadata.to_dict())
        write_dict_to_yaml(
            f"{self.writer.root}/{writer_yaml_name}", self.get_writing_info_dict())
    ...
    def _update_tqdm_message(self, pbar=tqdm(), batch_idx: int = 0):
        ...
        if (batch_idx == 0 or elapsed_time > 5):
            ...
            disk_size_available_gigabytes = np.round(
                disk_size_available_bytes/(1024**3), 2)

            # get size of dataset written so far
            dataset_size_current_gigabytes = self._get_directory_size_gigabytes(
                self.writer.root)
            # estimate size per sample
            dataset_size_per_sample_gigabytes = dataset_size_current_gigabytes / \
                (batch_idx+1)
