...
class ImageVisualizer(Visualizer):
    ...
    def _visualize(self, data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            ...
            iq_data = data[sample_idx]
            plt.imshow(
                iq_data,
                vmin=iq_data[iq_data != -np.inf].min(),
                vmax=iq_data[iq_data != np.inf].max(),
                aspect="auto",
                cmap="jet",
            )
            plt.xticks([])
...
class PSDVisualizer(Visualizer):
    ...
    def _visualize(self, iq_data: np.ndarray, targets: np.ndarray) -> Figure:
        ...
        for sample_idx in range(batch_size):
            ...
            _ = plt.psd(iq_data[sample_idx], NFFT=self.fft_size, Fs=1)
            plt.xticks(tuple())
            plt.yticks(tuple())
            plt.xlabel("")
            plt.ylabel("")
