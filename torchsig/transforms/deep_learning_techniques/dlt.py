...
from torchsig.transforms.signal_processing import Normalize
    ...
    def __call__(self, data: Any) -> Any:
        ...
        if isinstance(data, SignalData):
            ...
            for signal_desc in signal_description:
                ...
                # Update labels
                if new_signal_desc.start > insert_start and new_signal_desc.start < insert_start + insert_dur:
                    # Label starts within cut region
                    if new_signal_desc.stop > insert_start and new_signal_desc.stop < insert_start + insert_dur:
                        # Label also stops within cut region --> Remove label
                        continue
                    else:
                        # Push label start to end of cut region
                        new_signal_desc.start = insert_start + insert_dur
                elif new_signal_desc.stop > insert_start and new_signal_desc.stop < insert_start + insert_dur:
                    # Label stops within cut region but does not start in region --> Push stop to begining of cut region
                    new_signal_desc.stop = insert_start
                elif new_signal_desc.start < insert_start and new_signal_desc.stop > insert_start + insert_dur:
                    # Label traverse cut region --> Split into two labels
                    new_signal_desc_split = deepcopy(signal_desc)
                    # Update first label region's stop
                    new_signal_desc.stop = insert_start
            ...
            for insert_signal_desc in insert_signal_description:
                # Update labels
                if insert_signal_desc.stop < insert_start or insert_signal_desc.start > insert_start + insert_dur:
                    # Label is outside inserted region --> Remove label
                    continue
                elif insert_signal_desc.start < insert_start and insert_signal_desc.stop < insert_start + insert_dur:
            ...
            raise ValueError(
                "Expected input type `SignalData`. Received {}. \n\t\
                The `DatasetWidebandCutMix` transform depends on metadata from a `SignalData` object."
                .format(type(data))
            )
...
class DatasetWidebandMixUp(SignalTransform):
    ...
    def __call__(self, data: Any) -> Any:
        ...
        if isinstance(data, SignalData):
            ...
            raise ValueError(
                "Expected input type `SignalData`. Received {}. \n\t\
                The `DatasetWidebandMixUp` transform depends on metadata from a `SignalData` object."
                .format(type(data))
            )
...
class SpectrogramRandomResizeCrop(SignalTransform):
    ...
    def __call__(self, data: Any) -> Any:
        ...
        spec_data = eft_f.spectrogram(iq_data, nperseg, noverlap, nfft, self.window_fcn, self.mode)
        ...
        if isinstance(data, SignalData):
            ...
            for signal_desc in signal_description:
                ...
                if pad_height:
                    new_signal_desc.lower_frequency = ((new_signal_desc.lower_frequency+0.5)*curr_height + pad_height_start) / self.height - 0.5
                    new_signal_desc.upper_frequency = ((new_signal_desc.upper_frequency+0.5)*curr_height + pad_height_start) / self.height - 0.5
                    new_signal_desc.center_frequency = ((new_signal_desc.center_frequency+0.5)*curr_height + pad_height_start) / self.height - 0.5
                    new_signal_desc.bandwidth = new_signal_desc.upper_frequency - new_signal_desc.lower_frequency
                ...
                if crop_height:
                    ...
                    if (new_signal_desc.lower_frequency+0.5)*curr_height <= crop_height_start:
                        ...
                        new_signal_desc.lower_frequency = ((new_signal_desc.lower_frequency+0.5)*curr_height - crop_height_start) / self.height - 0.5
                    if (new_signal_desc.upper_frequency+0.5)*curr_height >= crop_height_start+self.height:
                        ...
                        new_signal_desc.upper_frequency = ((new_signal_desc.upper_frequency+0.5)*curr_height - crop_height_start) / self.height - 0.5
                    new_signal_desc.bandwidth = new_signal_desc.upper_frequency - new_signal_desc.lower_frequency
                    new_signal_desc.center_frequency = new_signal_desc.lower_frequency + new_signal_desc.bandwidth / 2
                    
                if pad_width:
                    new_signal_desc.start = (new_signal_desc.start * curr_width + pad_width_start) / self.width
                    new_signal_desc.stop = (new_signal_desc.stop * curr_width + pad_width_start) / self.width
                    new_signal_desc.duration = new_signal_desc.stop - new_signal_desc.start
                ...
                if crop_width:
                    if new_signal_desc.start*curr_width <= crop_width_start:
                        ...
                        new_signal_desc.start = (new_signal_desc.start * curr_width - crop_width_start) / self.width    
                    if new_signal_desc.stop*curr_width >= crop_width_start+self.width:
                        new_signal_desc.stop = 1.0
                    elif new_signal_desc.stop*curr_width <= crop_width_start:
                        continue
                    else:
                        new_signal_desc.stop = (new_signal_desc.stop * curr_width - crop_width_start) / self.width
                    new_signal_desc.duration = new_signal_desc.stop - new_signal_desc.start
