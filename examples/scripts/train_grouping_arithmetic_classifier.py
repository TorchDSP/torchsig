"""Train a tiny classifier from a TorchSIG loader and arithmetic grouping YAML.

Run from the repository root:

    python examples/scripts/train_grouping_arithmetic_classifier.py
"""
...
CONFIG_PATH = Path(__file__).with_name("grouping_arithmetic.yaml")
FFT_SIZE = 32
BATCH_SIZE = 8
TRAINING_STEPS = 40
VALIDATION_STEPS = 5
...
def dataset_metadata() -> dict:
    """Return metadata with a broad range of generated signal bandwidths."""
...
def build_dataloader(seed: int) -> tuple[WorkerSeedingDataLoader, GroupingLabel]:
    """Build a TorchSIG loader labeled by arithmetic bandwidth formulas."""
    grouping = GroupingLabel(CONFIG_PATH)
    dataset = TorchSigIterableDataset(
        signal_generators=["bpsk", "qpsk", "2fsk", "4gfsk"],
...
def train_classifier(
    ...
    """Train a small classifier from TorchSIG-generated batches."""
    torch.manual_seed(0)
    model = nn.Sequential(
...
def evaluate_classifier(
    ...
    """Evaluate on fresh TorchSIG-generated batches."""
    correct = 0
    total = 0
    batches = iter(dataloader)
...
def main() -> None:
    """Train using arithmetic labels returned by TorchSIG data loaders."""
