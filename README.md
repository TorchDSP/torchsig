...
docker run --ipc=host --ulimit memlock=-1 -u $(id -u ${USER}):$(id -g ${USER}) -v `pwd`:/workspace/code/torchsig torchsig python3 torchsig/scripts/generate_sig53.py -e HOME=/workspace/code/torchsig/examples/sig53 --root=/workspace/code/torchsig/examples/sig53 --all=True
```
...
```
docker build -t torchsig -f Dockerfile .
docker run --ipc=host --ulimit memlock=-1 -u $(id -u ${USER}):$(id -g ${USER}) -v `pwd`:/workspace/code/torchsig torchsig python3 torchsig/scripts/generate_wideband_sig53.py -e HOME=/workspace/code/torchsig/examples/sig53 --root=/workspace/code/torchsig/examples/wideband_sig53 --all=True
```
