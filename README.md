...
docker run --ipc=host --ulimit memlock=-1 -u $(id -u ${USER}):$(id -g ${USER}) -v `pwd`:/workspace/code/torchsig -e HOME=/workspace/code/torchsig/examples/sig53 torchsig python3 torchsig/scripts/generate_sig53.py --root=/workspace/code/torchsig/examples/sig53 --all=True
```
...
```
docker build -t torchsig -f Dockerfile .
docker run --ipc=host --ulimit memlock=-1 -u $(id -u ${USER}):$(id -g ${USER}) -v `pwd`:/workspace/code/torchsig -e HOME=/workspace/code/torchsig/examples/sig53 torchsig python3 torchsig/scripts/generate_wideband_sig53.py --root=/workspace/code/torchsig/examples/wideband_sig53 --all=True
```
...
```
docker build -t torchsig -f Dockerfile .
docker run -d --rm --network=host --shm-size=32g --gpus all --name torchsig_workspace -v `pwd`/examples:/workspace/code/examples -e HOME=/workspace/code/torchsig/examples/sig53torchsig tail -f /dev/null
